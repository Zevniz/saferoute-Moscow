...
MEASURED_TRAFFIC_FILE="${MEASURED_TRAFFIC_FILE:-}"
...
if [[ -z "$MEASURED_TRAFFIC_FILE" ]]; then
  fail "MEASURED_TRAFFIC_FILE is required and must point to a real licensed measured traffic CSV"
fi

if [[ ! -f "$MEASURED_TRAFFIC_FILE" ]]; then
  fail "MEASURED_TRAFFIC_FILE does not exist: $MEASURED_TRAFFIC_FILE"
fi
...
for required_name in DATASET_VERSION SOURCE_NAME SOURCE_OWNER SOURCE_URL SOURCE_LICENSE SOURCE_CHECKSUM EDGE_MAPPING_METHOD; do
  if [[ -z "${!required_name:-}" ]]; then
    fail "$required_name is required for measured traffic imports"
  fi
done

source_realpath="$("$PYTHON_BIN" - "$MEASURED_TRAFFIC_FILE" <<'PY'
...
case "$source_realpath" in
  */tests/fixtures/*)
    if [[ "$activation" == "true" ]]; then
      fail "test fixtures under tests/fixtures cannot be activated as production measured traffic data"
    fi
    ;;
esac
...
case "$metadata_text" in
  *osm*|*openstreetmap*|*maxspeed*|*lanes*|*highway*|*road_exposure_proxy*|*accident*|*crash*|*dtp*)
    fail "measured traffic import rejects OSM road-class/maxspeed/lanes, road-exposure proxies, and accident/crash sources"
    ;;
esac

if [[ "$activation" == "true" && "$license_confirmed" != "true" ]]; then
  fail "ACTIVATE_ENRICHMENT=true requires SOURCE_LICENSE_CONFIRMED=true for a licensed measured traffic export"
fi

actual_checksum="$("$PYTHON_BIN" - "$MEASURED_TRAFFIC_FILE" <<'PY'
...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/saferoute-measured-traffic.XXXXXX")"
...
cd "$ROOT_DIR"
"$PYTHON_BIN" scripts/data/validate-measured-layer-csv.py measured_traffic "$MEASURED_TRAFFIC_FILE" "$normalized_csv" "$validation_report" >/dev/null
